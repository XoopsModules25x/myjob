...
if (!$tpl->is_cached('db:myjob_atom.tpl')) {
    $sitename       = htmlspecialchars($xoopsConfig['sitename'], ENT_QUOTES);
    $email          = checkEmail($xoopsConfig['adminmail'], true);
    $slogan         = htmlspecialchars($xoopsConfig['slogan'], ENT_QUOTES);
    $module_handler = xoops_getHandler('module');
    $module         =& $module_handler->getByDirname('myjob');
    $category       = 'Job';
    $modulename     = $xoopsModule->getVar('name');
    $version        = $xoopsModule->getVar('version');
    $tpl->assign('charset', $charset);
    $tpl->assign('rights', $xoopsConfigMetaFooter['meta_copyright']);
    $tpl->assign('module_version', $version);
    ...
    $tpl->assign('channel_generator', xoops_utf8_encode($modulename));
    ...
    if ($useoffers) {
        ...
        foreach ($offres as $oneoffre) {
            $tpl->append('items', array(
                'category'    => htmlspecialchars(_MYJOB_OFFER),
                'title'       => xoops_utf8_encode(htmlspecialchars(_MYJOB_OFFER)) . xoops_utf8_encode(htmlspecialchars($oneoffre->getVar('titreannonce'), ENT_QUOTES)),
                'link'        => XOOPS_URL . '/modules/myjob/offer-view.php?offerid=' . $oneoffre->getVar('offreid'),
                'guid'        => XOOPS_URL . '/modules/myjob/offer-view.php?offerid=' . $oneoffre->getVar('offreid'),
                'pubdate'     => formatTimestamp($oneoffre->getVar('datevalidation'), 'rss'),
                'description' => xoops_utf8_encode(htmlspecialchars($oneoffre->getVar('description'), ENT_QUOTES))
            ));
        }
    }
    ...
    if ($usedemands) {
        ...
        foreach ($demandes as $onedemande) {
            ...
            $tpl->append('items', array(
                'category'    => _MYJOB_DEMAND,
                'title'       => _MYJOB_DEMAND . xoops_utf8_encode($titreannonce),
