...
if(is_object($xoopsUser) && $xoopsUser->isAdmin($xoopsModule->getVar('mid'))) {
	if ($demande->getVar('demandid')) {
		...
		if($demande->getVar('datevalidation','s')==0) {
			$ladate = (intval((time()-$demande->getVar('datesoumission'))/86400)>0) ? $ldate=sprintf(_MYJOB_DEMAND_WAITINGSINCE,intval((time()-$demande->getVar('datesoumission'))/86400)) : _MYJOB_DEMAND_TODAY;
		} else {
			$ladate=sprintf(_MYJOB_DEMAND_VALIDATED,formatTimestamp($demande->getVar('datevalidation','s')),intval((time()-$demande->getVar('datesoumission'))/86400));
		}
