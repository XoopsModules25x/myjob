...
if (!empty($demandid) && $usedemands) {
    ...
    if (myjob_fields('nom', false, 'demande')) {
        $content .= _MYJOB_DEMAND_NOM . ' : ' . $demande->getVar('nom') . '<br>';
    }
    if (myjob_fields('prenom', false, 'demande')) {
        $content .= _MYJOB_DEMAND_PRENOM . ' : ' . $demande->getVar('prenom') . '<br>';
    }
    if (myjob_fields('adresse', false, 'demande')) {
        $content .= _MYJOB_DEMAND_ADRESSE . ' : ' . $demande->getVar('adresse') . '<br>';
    }
    if (myjob_fields('cp', false, 'demande')) {
        $content .= _MYJOB_DEMAND_CP . ' : ' . $demande->getVar('cp') . '<br>';
    }
    if (myjob_fields('ville', false, 'demande')) {
        $content .= _MYJOB_DEMAND_VILLE . ' : ' . $demande->getVar('ville') . '<br>';
    }
    if (myjob_fields('telephone', false, 'demande')) {
        $content .= _MYJOB_DEMAND_TELEPHONE . ' : ' . $demande->getVar('telephone') . '<br>';
    }
    if (myjob_fields('email', false, 'demande')) {
        $content .= _MYJOB_DEMAND_EMAIL . ' : ' . $demande->getVar('email') . '<br>';
    }
    if (myjob_fields('datenaiss', false, 'demande')) {
        $content .= _MYJOB_DEMAND_DATENAISS . ' : ' . $demande->getVar('datenaiss') . '<br>';
    }
    if (myjob_fields('datesoumission', false, 'demande')) {
        $content .= _MYJOB_DEMAND_DATESOUMISSION . ' : ' . formatTimestamp($demande->getVar('datesoumission'), 's') . '<br>';
    }
    if (myjob_fields('dateexpiration', false, 'demande')) {
        $content .= _MYJOB_DEMAND_EXPIRATION . ' : ' . formatTimestamp($demande->getVar('dateexpiration'), 's') . '<br>';
    }
    if (myjob_fields('titreannonce', false, 'demande')) {
        $content .= _MYJOB_DEMAND_DESCRIPTION . ' : ' . $demande->getVar('titreannonce') . '<br>';
    }
    if (myjob_fields('diplome', false, 'demande')) {
        $content .= _MYJOB_DEMAND_DIPLOME . ' : ' . $demande->getVar('diplome') . '<br>';
    }
    if (myjob_fields('formation', false, 'demande')) {
        $content .= _MYJOB_DEMAND_FORMATION . ' : ' . $demande->getVar('formation') . '<br>';
    }
    if (myjob_fields('typeposte', false, 'demande')) {
        $content .= _MYJOB_DEMAND_TYPEPOSTE . ' : ' . $types[$demande->getVar('typeposte')] . '<br>';
    }
    ...
    if (myjob_fields('zonegeographique', false, 'demande')) {
        $content .= _MYJOB_DEMAND_ZONEGEOGRAPHIQUEA . ' : ' . $demande->getVar('zonegeographique') . '<br>';
    }
    if (myjob_fields('secteurid', false, 'demande')) {
        $content .= _MYJOB_DEMAND_SECTEURACTIVITE . ' : ' . $libsecteurs . '<br>';
    }
    if (myjob_fields('secteuractivite', false, 'demande')) {
        $content .= _MYJOB_DEMAND_SECTEURACTIVITEA . ' : ' . $demande->getVar('secteuractivite') . '<br>';
    }
    if (myjob_fields('experience', false, 'demande')) {
        $content .= _MYJOB_DEMAND_EXPERIENCE . ' : ' . $demande->getVar('libelle_experience') . '<br>';
    }
    if (myjob_fields('experiencedetail', false, 'demande')) {
        $content .= _MYJOB_OFFER_EXPERIENCEDETAIL . ' : ' . $demande->getVar('experiencedetail') . '<br>';
    }
    if (myjob_fields('datedispo', false, 'demande')) {
        $content .= _MYJOB_DEMAND_DATEDISPO . ' : ' . formatTimestamp($demande->getVar('datedispo'), 's') . '<br>';
    }
    if (myjob_fields('parain', false, 'demande')) {
        $content .= _MYJOB_DEMAND_PARAIN . ' : ' . $demande->getVar('parain') . '<br>';
    }
    if (myjob_fields('datevalidation', false, 'demande')) {
        $content .= _MYJOB_OFFER_DATEVALID . ' : ' . formatTimestamp($demande->getVar('datevalidation'), 's') . '<br>';
    }
    if (myjob_fields('langues', false, 'demande')) {
        $content .= _MYJOB_DEMAND_LANGUES . ' : ' . $demande->getVar('langues') . '<br>';
    }
    if (myjob_fields('zonelibre', false, 'demande')) {
        $content .= _MYJOB_DEMAND_ZONELIBRE . ' : ' . $demande->getVar('zonelibre') . '<br>';
    }
    if (myjob_fields('sitfam', false, 'demande')) {
        $content .= _MYJOB_DEMAND_SITFAM . ' : ' . $tblsitfam[$demande->getVar('sitfam')] . '<br>';
    }
    if (myjob_fields('competences', false, 'demande')) {
        $content .= _MYJOB_DEMAND_COMPETENCES . ' : ' . $demande->getVar('competences') . '<br>';
    }
    if (myjob_fields('divers', false, 'demande')) {
        $content .= _MYJOB_DEMAND_DIVERS . ' : ' . $demande->getVar('divers') . '<br>';
    }
    if (myjob_fields('hits', false, 'demande')) {
        $content .= _MYJOB_DEMAND_HITS . ' : ' . $demande->getVar('hits') . '<br>';
    }

    $author = '';
    if (myjob_fields('nom', false, 'demande')) {
        $author .= $demande->getVar('nom') . '<br>';
    }
    if (myjob_fields('prenom', false, 'demande')) {
        $author .= $demande->getVar('prenom') . '<br>';
    }
