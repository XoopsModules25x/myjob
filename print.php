...
function PrintOffer()
{
	...
	echo '<br /><a href="'.XOOPS_URL.'/">'.XOOPS_URL.'</a><br /><br />'._MYJOB_URLFOROFFER.' <br /><a href="'.XOOPS_URL.'/modules/'.$xoopsModule->dirname().'/offer-view.php?offerid='.$offre->getVar('offreid').'">'.XOOPS_URL.'/modules/myjob/offer-view.php?offerid='.$offre->getVar('offreid').'</a></td></tr></table></body></html>';
}
...
function PrintDemand()
{
	...
	echo '<tr valign="top" style="font:12px;"><td>';
	echo "<table border='0' width='100%' align='center'>";
	if(myjob_fields('nom',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_NOM.'</td><td>'.$demande->getVar('nom').'</td></tr>';
	if(myjob_fields('prenom',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_PRENOM.'</td><td>'.$demande->getVar('prenom').'</td></tr>';
	if(myjob_fields('adresse',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_ADRESSE.'</td><td>'.$demande->getVar('adresse').'</td></tr>';
	if(myjob_fields('cp',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_CP.'</td><td>'.$demande->getVar('cp').'</td></tr>';
	if(myjob_fields('ville',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_VILLE.'</td><td>'.$demande->getVar('ville').'</td></tr>';
	if(myjob_fields('telephone',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_TELEPHONE.'</td><td>'.$demande->getVar('telephone').'</td></tr>';
	if(myjob_fields('email',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_EMAIL.'</td><td>'.$demande->getVar('email').'</td></tr>';
	if(myjob_fields('datenaiss',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_DATENAISS.'</td><td>'.$demande->getVar('datenaiss').'</td></tr>';
	if(myjob_fields('datesoumission',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_DATESOUMISSION.'</td><td>'.formatTimestamp($demande->getVar('datesoumission'),'s').'</td></tr>';
	if(myjob_fields('dateexpiration',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_EXPIRATION.'</td><td>'.formatTimestamp($demande->getVar('dateexpiration'),'s').'</td></tr>';
	if(myjob_fields('titreannonce',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_DESCRIPTION.'</td><td>'.$demande->getVar('titreannonce').'</td></tr>';
	if(myjob_fields('diplome',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_DIPLOME.'</td><td>'.$demande->getVar('diplome').'</td></tr>';
	if(myjob_fields('formation',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_FORMATION.'</td><td>'.$demande->getVar('formation').'</td></tr>';
	if(myjob_fields('typeposte',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_TYPEPOSTE.'</td><td>'.$types[$demande->getVar('typeposte')].'</td></tr>';
	if(myjob_fields('zoneid',false,'demande')) {
		echo '<tr><td>'._MYJOB_DEMAND_ZONEGEOGRAPHIQUE.'</td><td>'.$libzones.'</td></tr>';
	}
	if(myjob_fields('zonegeographique',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_ZONEGEOGRAPHIQUEA.'</td><td>'.$demande->getVar('zonegeographique').'</td></tr>';
	if(myjob_fields('secteurid',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_SECTEURACTIVITE.'</td><td>'.$libsecteurs.'</td></tr>';
	if(myjob_fields('secteuractivite',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_SECTEURACTIVITEA.'</td><td>'.$demande->getVar('secteuractivite').'</td></tr>';
	if(myjob_fields('experience',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_EXPERIENCE.'</td><td>'.$demande->getVar('libelle_experience').'</td></tr>';
	if(myjob_fields('experiencedetail',false,'demande')) echo '<tr><td>'._MYJOB_OFFER_EXPERIENCEDETAIL.'</td><td>'.$demande->getVar('experiencedetail').'</td></tr>';
	if(myjob_fields('datedispo',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_DATEDISPO.'</td><td>'.formatTimestamp($demande->getVar('datedispo'),'s').'</td></tr>';
	if(myjob_fields('parain',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_PARAIN.'</td><td>'.$demande->getVar('parain').'</td></tr>';
	if(myjob_fields('datevalidation',false,'demande')) echo '<tr><td>'._MYJOB_OFFER_DATEVALID.'</td><td>'.formatTimestamp($demande->getVar('datevalidation'),'s').'</td></tr>';
	if(myjob_fields('langues',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_LANGUES.'</td><td>'.$demande->getVar('langues').'</td></tr>';
	if(myjob_fields('zonelibre',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_ZONELIBRE.'</td><td>'.$demande->getVar('zonelibre').'</td></tr>';
	if(myjob_fields('sitfam',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_SITFAM.'</td><td>'.$tblsitfam[$demande->getVar('sitfam')].'</td></tr>';
	if(myjob_fields('competences',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_COMPETENCES.'</td><td>'.$demande->getVar('competences').'</td></tr>';
	if(myjob_fields('divers',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_DIVERS.'</td><td>'.$demande->getVar('divers').'</td></tr>';
	if(myjob_fields('hits',false,'demande')) echo '<tr><td>'._MYJOB_DEMAND_HITS.'</td><td>'.$demande->getVar('hits').'</td></tr>';

	echo '</table>';
	echo '</td></tr></table></td></tr></table><br /><br />';
	printf(_MYJOB_THISCOMESFROM,$xoopsConfig['sitename']);
	echo '<br /><a href="'.XOOPS_URL.'/">'.XOOPS_URL.'</a><br /><br />'._MYJOB_URLFORDEMAND.' <br /><a href="'.XOOPS_URL.'/modules/myjob/demande-view.php?demandid='.$demande->getVar('demandid').'">'.XOOPS_URL.'/modules/myjob/demande-view.php?demandid='.$demande->getVar('demandid').'</a></td></tr></table></body></html>';
}
?>