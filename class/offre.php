...
class MyjobOffreHandler extends MjXoopsPersistableObjectHandler
{
    ...
    {    //                                         Table       Classe      Id
        parent::__construct($db, 'myjob_offre', 'offre', 'offreid');
    }

    public function updateCounter($offreid)
    {
        $sql = 'UPDATE ' . $this->db->prefix('myjob_offre') . ' SET hits=hits+1 WHERE offreid = ' . (int)$offreid;
    ...
    public function delete(XoopsObject $offre, $force = false)
    {
    ...
    public function unvalidate($offreid)
    {
        $sql = 'UPDATE ' . $this->db->prefix('myjob_offre') . ' SET online=0 WHERE offreid = ' . (int)$offreid;
        if ($this->db->queryF($sql)) {
            return true;
        } else {
            return false;
