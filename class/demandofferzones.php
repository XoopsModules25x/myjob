...
class demandofferzones extends MjObject
{
	function demandofferzones()
	{
		...
		$this->initVar('zoneid',XOBJ_DTYPE_INT,null,false);					// Id de la zone géographique
	}
}
...
{
	function MyjobDemandofferzonesHandler($db)
	{	//											Table						Classe				Id
		$this->MjXoopsPersistableObjectHandler($db, 'myjob_demandofferzones', 'demandofferzones', 'demandofferzoneid');
	}
	...
	*/
	function getTop($type='demandes',$limit=0, $start=0, $orderby='cpt',$desc='')
	{
		...
		$sql='SELECT z.zoneid, Count(z.zoneid) As cpt, s.libelle FROM '.$this->db->prefix('myjob_demandofferzones').' z LEFT JOIN '.$this->db->prefix('myjob_zonegeographique').' s ON z.zoneid=s.zoneid LEFT JOIN '.$this->db->prefix('myjob_demande').' d on z.demandid=d.demandid WHERE z.'.$field.'<>0 GROUP BY z.zoneid ORDER BY '.$orderby.' '.$desc;
		$result = $this->db->query($sql, $limit, $start);
		if (!$result) {
			return $ret;
		}
