...
{
	function MyjobDemandeHandler ($db)
	{	//											Table			Classe			Id
		$this->MjXoopsPersistableObjectHandler($db, 'myjob_demande', 'demande', 'demandid');
	}


	function &get($id)	{
		$sql = 'SELECT d.*, e.libelle as libelle_experience FROM '.$this->db->prefix('myjob_demande').' d LEFT OUTER JOIN '.$this->db->prefix('myjob_experience').' e ON d.experience = e.experienceid WHERE demandid='.intval($id);
	...
	function delete(&$demande, $force = false)
	{
	...
	*/
	function prolongate($id)
	{
		$duree = myjob_getmoduleoption('defaultduration') * 86400;
		$sql = 'UPDATE '.$this->db->prefix('myjob_demande').' SET dateexpiration=dateexpiration+'.$duree.' WHERE demandid='.intval($id);
		$result = $this->db->queryF($sql);
	}
	...
	*/
	...
	{
		$sql = 'UPDATE '.$this->db->prefix('myjob_demande').' SET contacts=contacts+1 WHERE demandid='.intval($id);
		$result = $this->db->queryF($sql);
	}
	...
	*/
	function &getAllProlongationsDemands()
	{
		$ret=array();
		// TODO: A vérifier
		$sql = 'SELECT '.$this->db->prefix('myjob_demande').".*, e.libelle as libelle_experience  FROM ".$this->db->prefix('myjob_prolongation').' LEFT OUTER JOIN '.$this->db->prefix('myjob_demande').' ON '.$this->db->prefix('myjob_prolongation').'.demandid='.$this->db->prefix('myjob_demande').'.demandid';
	...
	*/
	function getFilteredDemands($criteria = null, $id_as_key = false, $count=false)
	{
		$ret = array();
		$limit = $start = 0;
		if($count) {
		...
		$sql = 'SELECT '.$what.' FROM '.$this->db->prefix('myjob_demande').' d JOIN '.$this->db->prefix('myjob_demandoffersecteurs').' s ON d.demandid=s.demandid JOIN '.$this->db->prefix('myjob_demandofferzones').' z ON d.demandid=z.demandid JOIN '.$this->db->prefix('myjob_typeposte').' p ON d.typeposte = p.typeid JOIN '.$this->db->prefix('myjob_experience').' e ON d.experience = e.experienceid ';
		if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
			$sql .= ' '.$criteria->renderWhere();
			if ($criteria->getSort() != '') {
				$sql .= ' ORDER BY '.$criteria->getSort().' '.$criteria->getOrder();
	...
	*/
	function getRandomDemands($criteria = null, $id_as_key = false)
	{
		$ret = $rand_keys = $ret3 = array();
		$limit=$start=0;
		$sql = 'SELECT distinct d.*, e.libelle as libelle_experience FROM '.$this->db->prefix('myjob_demande').' d LEFT OUTER JOIN '.$this->db->prefix('myjob_demandoffersecteurs').' s ON d.demandid=s.demandid LEFT OUTER JOIN '.$this->db->prefix('myjob_demandofferzones').' z ON d.demandid=z.demandid LEFT OUTER JOIN '.$this->db->prefix('myjob_typeposte').' p ON d.typeposte = p.typeid ';
	 ...
	 */
	function updateCounter($demandid)
	{
		$sql = 'UPDATE ' . $this->db->prefix('myjob_demande') . ' SET hits=hits+1 WHERE demandid = ' . intval($demandid);
	 ...
	 */
	function unvalidate($demandid)
	{
		$sql = 'UPDATE ' . $this->db->prefix('myjob_demande') . ' SET datevalidation=0 WHERE demandid = ' . intval($demandid);
		if ($this->db->queryF($sql)) {
			return true;
		} else {
			return false;
	 ...
	 */
	function getTopTypeContrat($limit=0, $start=0, $orderby='cpt',$desc='')
	{
		$ret = array();
		$ts =& MyTextSanitizer::getInstance();
		$sql='SELECT COUNT(*) AS cpt, d.typeposte, t.libelle FROM '.$this->db->prefix('myjob_demande').' d LEFT OUTER JOIN '.$this->db->prefix('myjob_typeposte').' t ON d.typeposte= t.typeid WHERE d.datevalidation<>0 AND d.dateexpiration > '.time().' GROUP BY d.typeposte ORDER BY '.$orderby.' '.$desc;
