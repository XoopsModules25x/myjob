...
switch($op)
{
	case 'post':
		if(empty($offreid)) {
			$offreid= isset($_POST['offreid']) ? intval($_POST['offreid']) : 0;
		}

		if(!empty($offreid)) {	// Mode édition
			$offre = $offre_handler->get($offreid);
			$offre->setVars($_POST);
			$offre->unsetNew();
			if(isset($_POST['online']) && intval($_POST['online'])==1 && $admin) {
				$offre->setVar('datevalidation',time());
				$offre->setVar('approver',$xoopsUser->getVar('uid'));
			}

			if(isset($_POST['online']) && $admin) {
				$offre->setVar('online',intval($_POST['online']));
				if(intval($_POST['online'])==1 && xoops_trim($offre->getVar('email'))!='') {	// Notify the author of the publication
