...
switch($op) {
	case 'post':
		...
		if(empty($demandid)) {
			$demandid= isset($_POST['demandid']) ? intval($_POST['demandid']) : 0;
		}
		...
		if(myjob_getmoduleoption('uploaddemands')) { // Manage upload(s)
			if(isset($_POST['xoops_upload_file'])) {
				...
				if(xoops_trim($fldname!='')) {
					...
					$permittedtypes=array('image/gif', 'image/jpeg', 'image/pjpeg', 'image/x-png', 'image/png' ,'application/x-zip-compressed','application/zip', 'application/pdf', 'application/x-gtar', 'application/x-tar', 'application/msword', 'application/vnd.ms-excel', 'application/vnd.oasis.opendocument.text', 'application/vnd.oasis.opendocument.spreadsheet', 'application/vnd.oasis.opendocument.presentation', 'application/vnd.oasis.opendocument.graphics', 'application/vnd.oasis.opendocument.chart', 'application/vnd.oasis.opendocument.formula', 'application/vnd.oasis.opendocument.database', 'application/vnd.oasis.opendocument.image', 'application/vnd.oasis.opendocument.text-master');
		...
		if($res!=false) {		// La sauvegarde s'est bien déroulée
			...
			if(myjob_getmoduleoption('autoapprovedemands')) {	// Dans le cas de l'auto-approbation
	    		if(isset($_POST['adminside']) && intval($_POST['adminside'])==1) {
